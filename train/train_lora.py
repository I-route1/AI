"""
수학 LoRA 파인튜닝 - 순수 PyTorch 학습 루프 (Trainer/trl 미사용)

실행: .\venv_blackwell\Scripts\python.exe train/train_lora.py
출력: train/math_adapter/
"""
...
OUTPUT_DIR     = "train/math_adapter"
MAX_LENGTH     = 512         # 512로 축소 (수학 문제는 대부분 충분)
BATCH_SIZE     = 1           # OOM 방지
...
def main():
    ...
    print("학습 데이터 로드 중...")
    train_dataset = MathDataset(TRAIN_FILE, tokenizer, MAX_LENGTH)
    print("평가 데이터 로드 중...")
    eval_dataset  = MathDataset(EVAL_FILE,  tokenizer, MAX_LENGTH)
    ...
    print(f"\n학습 시작! (총 {total_steps}스텝, warmup {warmup_steps}스텝)")
