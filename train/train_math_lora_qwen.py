"""
수학 LoRA 파인튜닝 - Qwen3-8B 버전 (순수 PyTorch 학습 루프)

기존 Gemma4 버전(train_lora.py) 대비 변경점:
- 베이스 모델을 Qwen3-8B(4bit)로 교체 - 글쓰기 어댑터와 같은 베이스를 써서
  서버에서 베이스 모델 하나로 어댑터만 바꿔가며(set_adapter) 수학/글쓰기를
  같이 서빙할 수 있게 함 (기존엔 서로 다른 베이스라 VRAM 두 배로 들어 동시 로드 불가했음)
- 데이터(math_train.jsonl/math_eval.jsonl)는 이미 학습 가능한 형식이라 그대로 재사용
- loss를 assistant(정답+풀이) 토큰에만 계산하도록 프롬프트 구간 라벨 마스킹 추가

실행: .\venv_blackwell\Scripts\python.exe train/train_math_lora_qwen.py
출력: train/math_adapter_qwen/
"""
...
TRAIN_FILE     = "train/math_train.jsonl"
EVAL_FILE      = "train/math_eval.jsonl"
OUTPUT_DIR     = "train/math_adapter_qwen"
MAX_LENGTH     = 640         # 수학 데이터는 짧음 (p99=513) - 640이면 사실상 전부 커버
BATCH_SIZE     = 1
GRAD_ACCUM     = 16
NUM_EPOCHS     = 2
LR             = 2e-4
