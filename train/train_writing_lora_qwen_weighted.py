"""
글쓰기 피드백 LoRA 파인튜닝 - Qwen3-8B + 채점 클래스 가중 손실 버전

train_writing_lora_qwen.py 대비 변경점:
- 원본 채점 데이터가 3점(57%)에 극도로 쏠려있어 모델이 "3"으로 회귀하는
  경향을 보임 (1점 2.7%, 5점 0%). 데이터를 덜어내는 대신, 학습 데이터 구성은
  그대로 두고 손실(loss) 계산 시 희귀 점수 클래스에 더 큰 가중치를 부여해서
  교정한다 (class-balanced weighting, sklearn의 'balanced' 방식).
- 채점(숫자) 샘플에만 가중치가 적용되고, 피드백(서술형) 샘플은 가중치 1.0
  (기존과 동일하게 취급)
- 평가(eval) 손실은 가중치 없이 원래 그대로 계산 - 다른 실행과 비교 가능하게
- 완전히 새로운 학습(처음부터)이라 OUTPUT_DIR을 별도로 둠(기존 무가중 어댑터
  보존 목적)

전처리: python train/preprocess_writing_qwen.py (writing_train_qwen.jsonl 재사용)
실행:   .\venv_blackwell\Scripts\python.exe train/train_writing_lora_qwen_weighted.py
출력:   train/writing_adapter_qwen_weighted/
"""
...
OUTPUT_DIR     = "train/writing_adapter_qwen_weighted"
MAX_LENGTH     = 896
BATCH_SIZE     = 1
GRAD_ACCUM     = 16
NUM_EPOCHS     = 3
...
EVAL_STEPS     = 100
SAVE_STEPS     = 200
MAX_CLASS_WEIGHT = 5.0   # 희귀 클래스(1점) 가중치가 지나치게 커지는 것 방지
